...
public class Buscaminas {
    ...
    public Buscaminas(int[] args) {
        int filas = (args[0] < 0 ? 1 : Math.min(args[0], 20));
        int columnas = (args[1] < 8 ? 8 : Math.min(args[1], 30));
        int minas = (args[2] < 0 ? 0 : Math.min(args[2], filas * columnas - 1));
    ...
    private void colocarMinas(int[] casilla) {
        for (int i = 0; i < CANTIDAD_MINAS; i++) {
            while (true) {
                ...
                if (!esMina(casillaAleatoria) && ((tablero.length * tablero[0].length - CANTIDAD_MINAS < 9) ? !Arrays.equals(casilla, casillaAleatoria) : (!(casillaAleatoria[0] >= casilla[0] - 1 && casillaAleatoria[0] <= casilla[0] + 1) || !(casillaAleatoria[1] >= casilla[1] - 1 && casillaAleatoria[1] <= casilla[1] + 1)))) {
